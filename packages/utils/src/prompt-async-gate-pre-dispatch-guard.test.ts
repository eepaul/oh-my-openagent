...
describe("dispatchInternalPrompt preDispatchGuard", () => {
  afterEach(() => {
    // then
    releaseAllPromptAsyncReservationsForTesting()
  })

  test("#given no preDispatchGuard #when the gate dispatches #then its existing result and prompt side effect remain unchanged", async () => {
    // given
    let promptCalls = 0
    const client = {
      session: {
  ...
  test("#given a rejecting preDispatchGuard #when the session is otherwise ready #then promptAsync is not called", async () => {
    // given
    let promptCalls = 0
    const client = {
      session: {
    ...
    const result = await dispatchInternalPrompt({
      mode: "async",
      client,
      sessionID: "ses_guard_rejected",
      input: createInput("ses_guard_rejected"),
      source: "test:guard-rejected",
      settleMs: 0,
      postDispatchHoldMs: 0,
      preDispatchGuard: () => false,
    })

    // then
    expect(result).toEqual({ status: "guard_rejected" })
