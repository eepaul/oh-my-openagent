...
export async function buildSubagentCompletionReminder(input: {
  ...
  const shouldPause = input.shouldPauseForApproval || isMissingFinalWaveVerdict || isRejectedFinalWaveVerdict
  ...
  if (isMissingFinalWaveVerdict) {
    ...
    return {
      ...
      shouldPauseForApproval: input.shouldPauseForApproval,
    }
  }
  ...
  if (input.shouldPauseForApproval) {
    return {
